...
function useStudentInfoAction(param) {
  ...
  async function getStudentList(Class) {
    ...
    const response = await fetchWrapper.get(
      `http://localhost:3000/api/classes/${Class.class_id}/members/infors`,
      null,
      null,
    );
    if (response == null) {
  ...
  async function deleteStudent(Id) {
    ...
    const response = await fetchWrapper.delete(
      `http://localhost:3000/api/classes/${currentClass.class_id}/members/delete`,
  ...
  async function AddStudent(emails) {
    ...
    const response = await fetchWrapper.post(
      `http://localhost:3000/api/classes/${currentClass.class_id}/members/add`,
      "application/x-www-form-urlencoded",
      urlencoded,
    );
    if (response == null) {
