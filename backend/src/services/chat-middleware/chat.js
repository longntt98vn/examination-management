...
async function fGetMessageByVNUId(req, res) {
  ...
  if (otherInstance) {
    ...
    res.status(200);
    if (chatRoom) res.send(Configs.RES_FORM("Success", chatRoom.messages));
    else {
    ...
    res.status(404);
    res.json(
      Configs.RES_FORM("Error", "Không tìm thấy đối tượng cần xem tin nhắn"),
    );
  }
}
...
async function fGetRecentContact(req, res) {
  ...
  for (var i of chatRooms) {
    ...
    if (
      i.membersID[0]._id.toHexString() == req.senderInstance._id.toHexString()
    )
      contacts.push({
