...
async function editProfileById(req, res) {
  ...
  if (req.body.new_password && req.body.old_password) {
    ...
    if (new_password.length < 8) {
      res.status(400);
      res.json(
        Configs.RES_FORM("Error", "Password mới chưa đủ độ dài (8 ký tự)"),
      );
      return;
    }
    if (hash(old_password) != senderLoginInfor.password) {
      res.status(400);
      res.json(
        Configs.RES_FORM("Error", "Password cũ và mới không trùng nhau"),
      );
      return;
    }
  ...
  global.DBConnection.User.findOneAndUpdate(
