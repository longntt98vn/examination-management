  ...
  if (!feedInstance) {
    ...
    try {
      ...
      res.json(
        Configs.RES_FORM(
          "Error",
          "Error when creating feed for class. Err:" + e.toString(),
        ),
      );
      return;
    }
...
async function fPostToFeed(req, res) {
  ...
  try {
    ...
    res.json(
      Configs.RES_FORM(
        "Error",
        "Error when creating new post to class feed. Err: " + e.toString(),
      ),
    );
    return;
  }
  try {
    ...
    res.json(
      Configs.RES_FORM(
        "Error",
        "Error when push post to feed instance. Err : " + e.toString(),
      ),
    );
  }
  try {
...
async function fCommentToPost(req, res) {
  ...
  try {
    ...
    res.json(
      Configs.RES_FORM(
        "Error",
        "Error when creating comment. Err: " + e.toString(),
      ),
    );
    return;
  }
  try {
    ...
    global.IOConnection.notifyNewComment(
      newComment,
      req.params.postId,
    ...
    res.json(
      Configs.RES_FORM(
        "Error",
        "Error when pushing comment to post. Err: " + e.toString(),
      ),
    );
    return;
  }
...
async function fLikePost(req, res) {
  ...
  try {
    ...
    res.status(400);
    res.json(
      Configs.RES_FORM("Error", "Lỗi khi like post. Err: " + e.toString()),
    );
    return;
  }
