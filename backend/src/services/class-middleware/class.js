...
function validateClassMember(req, res, next) {
  ...
  if (
    classInstance.class_members.includes(req.senderInstance._id) ||
    classInstance.class_teacher.equals(req.senderInstance._id)
  ) {
    next();
  } else {
    res.status(400);
...
async function fDeleteMemberInClass(req, res) {
  ...
  res.status(200);
  res.json(
    Configs.RES_FORM("Success", { deleted: deleted, failed: membersVNUId }),
  );
}
