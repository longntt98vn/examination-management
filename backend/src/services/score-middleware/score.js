  ...
  if (instanceUser && instanceSubject && instanceSemester) {
    ...
    if (instanceTable) {
      ...
      if (!subjectScoreExisted) next();
        ...
        res.status(400);
        res.json(
          RES_FORM("Error", "Điểm cho môn học này đã tồn tại trong bảng điểm"),
        );
        return;
      }
    } else {
    ...
    if (!instanceSemester) {
      res.json(
        RES_FORM("Error", "Không tìm thấy kỳ học " + req.body.semester_id),
      );
      return;
    }
    if (!instanceSubject) {
      res.json(
        RES_FORM("Error", "Không tìm thấy môn học" + req.body.subject_code),
      );
    }
...
async function fAddScoreToScoresTable(req, res) {
  ...
  try {
    ...
    res.status(400);
    res.json(
      RES_FORM("Error", `Lỗi khi nhập dữ liệu điểm vào bảng điểm. Lỗi: ${e}`),
    );
    return;
  }
}
