...
export const handleChatMessage = async (socket: any, msg: any) => {
    console.log('New Chat Message: ' + msg.toString());
    let message, to, from;
    try {
        var toInstance = await global.DBConnection.User.findOne({
            vnu_id: msg.to,
        });
    ...
    try {
        var chatRoom = await global.DBConnection.Chat.findOne({
            membersID: {
                $size: 2,
                $all: [
                    new ObjectId(socket.loginInfo.user_ref._id),
                    new ObjectId(toInstance._id),
                ],
            },
        });
        if (!chatRoom) {
    ...
    (newFullyMessage as any).newContact = newContact;
    try {
        var targetLoginInfo = await global.DBConnection.LoginInfo.findOne({
            user_ref: toInstance._id,
        });
