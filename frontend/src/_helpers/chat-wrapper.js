...
function useChatWrapper(param) {
  ...
  const [curListMessages, setCurListMessage] =
    useRecoilState(curListMessagesAtom);
  const [curListContact, setCurListContact] =
    useRecoilState(curListContactAtom);
  const [waitingToAddContact, setWaitingToAddContact] =
    useRecoilState(waitToAddContactAtom);
  const [listUpdateLatestMsg, setListUpdateLatestMsg] = useRecoilState(
    waitForUpdateLatestMsgAtom,
  );
  async function getRecentContact() {
    try {
      let res = await fetchWrapper.get(
        Configs.HOST_NAME + Configs.API_PATH.RECENT_CONTACT,
      );
      if (res.ok) {
        res = await res.json();
  ...
  async function fetchCurListMessageById(vnu_id) {
    try {
      ...
      let res = await fetchWrapper.get(
        Configs.HOST_NAME +
          Configs.API_PATH.MESSAGES_BY_VNU_ID.replace(":otherVNUId", vnu_id),
      );
      if (res.ok) {
        res = await res.json();
