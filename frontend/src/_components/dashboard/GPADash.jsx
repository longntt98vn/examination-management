...
function GPADash(props) {
  ...
  const [levelGPAstate, setlevelGPAState] = useRecoilState(
    dashboardLevelGPAAtom,
  );
  ...
  useEffect(() => {
    ...
    async function getSemesterData() {
      ...
      let response = await fetchWrapper.get(
        "http://localhost:3000/api/semesters/all",
        null,
        null,
      );
    ...
    async function initGPADash() {
            ...
            for (const object of props.score) {
              ...
              scoreObj.forEach((scoreElement) => {
                if (semFilterState == "nofilter") {
                  ...
                  totalScore +=
                    scoreElement.subject.credits_number * scoreElement.score;
                } else {
              ...
              if (
                ...
                (filterState == "nofilter" ||
                  (filterState == "cchv" && GPA < 2.5) ||
                  (filterState == "duoihoc" && GPA < 1))
              ) {
                refinedData.push({
                  vnu_id: vnu_id,
                  name: name,
                ...
                tempLevel.Aplus =
                  GPA <= 4.0 && GPA >= 3.7
                    ...
                    : tempLevel.Aplus;
                tempLevel.A =
                  GPA < 3.7 && GPA > 3.5 ? tempLevel.A + 1 : tempLevel.A;
                tempLevel.Bplus =
                  GPA <= 3.5 && GPA > 3.0
                    ? tempLevel.Bplus + 1
                    : tempLevel.Bplus;
                tempLevel.B =
                  GPA <= 3 && GPA > 2.5 ? tempLevel.B + 1 : tempLevel.B;
                tempLevel.Cplus =
                  GPA <= 2.5 && GPA > 2 ? tempLevel.Cplus + 1 : tempLevel.Cplus;
                tempLevel.C =
                  GPA <= 2 && GPA > 1.5 ? tempLevel.C + 1 : tempLevel.C;
                tempLevel.Dplus =
                  GPA <= 2 && GPA > 1.5 ? tempLevel.Dplus + 1 : tempLevel.Dplus;
                tempLevel.D =
                  GPA <= 1.5 && GPA >= 1 ? tempLevel.D + 1 : tempLevel.D;
                tempLevel.F =
                  GPA < 1 && GPA > 0 ? tempLevel.F + 1 : tempLevel.F;
              }
      ...
      var levelVisualized = [];
      levelVisualized.push(
        { name: "A+", value: tempLevel.Aplus },
        { name: "A", value: tempLevel.A },
      );
      levelVisualized.push(
        { name: "B+", value: tempLevel.Bplus },
        { name: "B", value: tempLevel.B },
      );
      levelVisualized.push(
        { name: "C+", value: tempLevel.Cplus },
        { name: "C", value: tempLevel.C },
      );
      levelVisualized.push(
        { name: "D+", value: tempLevel.Dplus },
        { name: "D", value: tempLevel.D },
      );
            ...
            <List
              ...
              renderItem={(item) => (
                <List.Item key={item.vnu_id}>
                  <List.Item.Meta title={item.name} description={item.gpa} />
                  <div>{item.GPA}</div>
                </List.Item>
              )}
            />
