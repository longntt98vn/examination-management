...
function useStudentScoreAction(param) {
  ...
  async function getScoreList(Class) {
    ...
    const response = await fetchWrapper.get(
      `http://localhost:3000/api/classes/${Class.class_id}/members/scores`,
      null,
      null,
    );
    if (response == null) {
  ...
  async function getScoreByID(vnu_id) {
    ...
    const response = await fetchWrapper.get(
      `http://localhost:3000/api/scores/${vnu_id}`,
      null,
      null,
    );
    if (response == null) {
  ...
  async function getSemesterByID(semesterId) {
    ...
    const response = await fetchWrapper.get(
      `http://localhost:3000/api/semesters/${semesterId}`,
      null,
      null,
    );
    if (response == null) {
  ...
  function handleData(data) {
    ...
    if (data != null) {
      data.forEach((object) => {
        ...
        var totalCredit = 0;
        var totalScore = 0;
        var date_of_birth = (object.user_ref.date_of_birth = moment
          .utc(object.user_ref.date_of_birth)
          .format("DD/MM/YYYY"));
        ...
        scoreObj.forEach((scoreElement) => {
          ...
          tempLevel.Cplus =
            score <= 2.5 && score > 2 ? tempLevel.Cplus + 1 : tempLevel.Cplus;
          tempLevel.C =
            score <= 2 && score > 1.5 ? tempLevel.C + 1 : tempLevel.C;
          tempLevel.Dplus =
            score <= 2 && score > 1.5 ? tempLevel.Dplus + 1 : tempLevel.Dplus;
          tempLevel.D =
            score <= 1.5 && score >= 1 ? tempLevel.D + 1 : tempLevel.D;
