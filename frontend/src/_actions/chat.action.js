  ...
  function findContactInList(vnu_id) {
    ...
    if (chatWrapper.curListContact)
      for (var i of chatWrapper.curListContact) {
        if (i.contact.vnu_id === vnu_id) return { element: i, index: count };
        count++;
      }
  ...
  async function addContactToList(obj) {
    ...
    try {
      let res = await fetcher.get(
        Configs.HOST_NAME +
          Configs.API_PATH.PROFILE_BY_ID.replace(":profileId", vnu_id),
      );
      if (!res.ok) {
        // debugger
      ...
      let newList = chatWrapper.curListContact
        ? [...chatWrapper.curListContact]
        : [];
      newList.push(form);
      ...
      setAlert({
        message: "Loi",
        description: "Khong the them contact. Err : " + e.toString(),
      });
    }
  }
