...
function useSocketAction() {
  ...
  function onUpdatePost(newPost) {
    ...
    setRecoil(feedPageAtom, {
      ...feedPageWrapper.feedPageState,
      posts: newPosts,
    });
  }
  function onNewMessage(message) {
    ...
    if (!message.selfSend) {
      setAlert({
        message: `Tin nhắn mới từ: ${message.from.name}`,
        description: message.message,
      });
    }

    if (
      message.from.vnu_id == getRecoil(chatWrapper.curChatPersonAtom) ||
      message.to.vnu_id == getRecoil(chatWrapper.curChatPersonAtom)
    ) {
    ...
    if (message.newContact && !message.isSender) {
      let listToAdd = getRecoil(waitToAddContactAtom)
        ? [...getRecoil(waitToAddContactAtom)]
        : [];
    ...
    let listUpdating = getRecoil(waitForUpdateLatestMsgAtom)
      ? [...getRecoil(waitForUpdateLatestMsgAtom)]
      : [];
