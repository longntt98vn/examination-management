...
async function validateToken(req, res, next) {
  ...
  try {
    ...
    var instance = await global.DBConnection.LoginInfo.findOne({
      current_token: token,
    }).populate("user_ref");
    ...
    if (err.name == "TokenExpiredError") {
      ...
      res.status(400);
      res.send(
        Configs.RES_FORM("Error", { name: err.name, description: err.message }),
      );
...
async function fForgetPassword(req, res) {
  ...
  let temp = await global.DBConnection.LoginInfo.findOneAndUpdate(
