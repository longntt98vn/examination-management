...
async function register(req, res) {
  let dupVNUId = await global.DBConnection.User.findOne({
  ...
  if (dupVNUId) {
    res.status(409);
    res.json(
      Configs.RES_FORM("Error", "VNU-ID is already registered by someone"),
    );
  } else {
    try {
      ...
      let newToken = jwt.sign(
        { vnu_id: newUserLoginInfo.vnu_id, createdDate: new Date().getTime() },
        Configs.SECRET_KEY,
        { expiresIn: 3600 },
      );
